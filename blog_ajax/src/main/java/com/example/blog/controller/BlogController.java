...
//    }
    ...
    @ResponseBody
    public List<Map<String, String>> suggestBlogTitles(
            @RequestParam String keyword,
            @RequestParam(required = false) Integer type) {

        List<BlogContent> results = blogService.findByTitleContainingIgnoreCase(keyword);

        if (type != null) {
            results = results.stream()
                    .filter(blog -> blog.getBlogType() != null
                            && blog.getBlogType().getId() == type)
                    .toList();
        }

        return results.stream()
                .limit(5)
                .map(blog -> Map.of("title", blog.getTitle()))
                .toList();
    }

    @GetMapping("/blog/loadMore")
    public String loadMoreBlogs(
            @RequestParam(name = "type", required = false, defaultValue = "1") Integer typeId,
            @RequestParam(name = "page", required = false, defaultValue = "0") int page,
            Model model) {

        Pageable pageable = PageRequest.of(page, 3, Sort.by("createdAt").descending());
        var blogPage = blogService.findAllByBlogType_Id(typeId, pageable);

        model.addAttribute("blogList", blogPage.getContent()); // ✅ Lấy list thực tế
        return "layout/blog-list :: blog-list";
    }
